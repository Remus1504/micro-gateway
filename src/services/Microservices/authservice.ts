...
import { IAuth } from '@remus1504/micrograde';

export let axiosAuthInstance: ReturnType<typeof axios.create>;

class AuthService {
  // Initallizse an instanse of the class Every microservice will have its own instance of axious to make requests.
  ...
  async getCourses(
    ...
    const response: AxiosResponse = await this.axiosService.axios.get(
      `/search/courses/${from}/${size}/${type}?${query}`,
    );
    return response;
  }
